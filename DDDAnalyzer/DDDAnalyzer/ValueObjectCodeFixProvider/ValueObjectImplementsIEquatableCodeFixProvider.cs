...
namespace DDDAnalyzer.ValueObjectCodeFixProvider
{
    ...
    public class ValueObjectImplementsIEquatableCodeFixProvider : Microsoft.CodeAnalysis.CodeFixes.CodeFixProvider
    {
        ...
        public sealed override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create(ValueObjectAnalyzer.ValueObjectsMustImplementIEquatableId);
        public sealed override FixAllProvider GetFixAllProvider() => WellKnownFixAllProviders.BatchFixer;
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            var root = await context.Document.GetSyntaxRootAsync(context.CancellationToken).ConfigureAwait(false);

            var diagnostic = context.Diagnostics.First(it => it.Id.Equals(ValueObjectAnalyzer.ValueObjectsMustImplementIEquatableId));
